...
The command must return zero after `restic check`; its final output includes the manifest and latest tagged snapshot. Retention filters the `hmqa` backup set and groups by tag, so disposable container hostnames and per-run temporary paths cannot create immortal retention groups. Record snapshot ID, timestamp, database size/checksum, object count, restic check result, and alert status. Schedule the same container with the platform scheduler; do not keep the only schedule inside the application process.
