...
| R-004 Versioned journal configuration and requirements | PRD 6, 11, 14 | Database schema, configuration API/admin | Clean-DB integration/E2E verified immutable publication and exact pinned requirement version | VERIFIED |
...
| R-010 Staff authentication, 2FA, RBAC, journal scopes | PRD 3, 10, 16 | API auth/security, admin | Real HTTP/Redis/PostgreSQL TOTP, lockout, session, CSRF, exact role/resource and IDOR tests; admin probe | VERIFIED |
| R-011 Complete editorial admin | PRD 10-11; request | `apps/admin-web`, admin API | All-page authenticated rendered probe and editorial API/E2E coverage; Academy browser UAT pending | TESTED |
| R-012 Reviews, assignments, deadlines, revisions | PRD 7.4, 8, 10 | Database, workflow/API/admin | Real editorial E2E covered assignment/review/revision/acceptance/publication and independent rejection | VERIFIED |
...
| R-016 Security controls and minimized logs | PRD 16; request | Security/config/logger/proxy | Unit/integration/runtime security gates, public history/secret/dependency scans and CodeQL with zero open alerts | VERIFIED |
...
| R-020 CI quality/security/migration gates | PRD 17.3; request | GitHub Actions | Main CI run 34313143644 passed fail-closed quality/integration/runtime/required jobs | VERIFIED |
| R-021 Production schema, migrations, constraints, indexes | PRD 14; request | `packages/database` | Nine migrations from zero, drift/status check and real constraint/transaction tests | VERIFIED |
...
| R-025 Operations documentation and runbooks | PRD 18, 20 | root public docs and `docs` | Public-repository audit and release-readiness self-review | VERIFIED |
